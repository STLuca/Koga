...
public class ReferenceParser implements Parser {
    ...
    Token DEPENDENCIES, IMPORTS, CONSTRUCTOR, NAME, IMPLEMENTS, OP_BRACE, CL_BRACE, OP_PAREN, CL_PAREN,
            ...
            CL_SQ_BRACKET, EQUALS, DOT, OP_PT_BRACE, CL_PT_BRACE, STRING, OPERATOR;
    ...
    public void parse(Classes classes, String input) {
        ...
        ReferenceCompilable c = new ReferenceCompilable();
    ...
    private void parseMethod(Scanner scanner, ReferenceCompilable c) {
        Token curr = scanner.current();
        Method m = new Method();
        m.name = curr.matched();
    ...
    private void parseStatement(Scanner scanner, ReferenceCompilable c, List<Statement> statements) {
        Token curr = scanner.current();
        if (curr != NAME) scanner.fail("name");
    ...
    private void parseMethodArguments(Scanner scanner, ReferenceCompilable c, Statement s) {
        Token curr = scanner.current();
    ...
    private void parseVariable(Scanner scanner, ReferenceCompilable c) {
        Field f = new Field();
        Token curr = scanner.current();
