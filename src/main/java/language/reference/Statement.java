...
public class Statement {
    ...
    void handle(
        ...
        for (Argument arg : arguments) {
            if (arg.literal != null) {
                ...
                Block b = new Block(arg.block, classes, variables);
        ...
        if (implicit != null) {
            args.add(language.core.Argument.of(implicit));
        }
        ...
        switch (type) {
            ...
            case CONSTRUCT -> {
                ...
                variable.clazz = classes.usable(clazz);
                variables.put(variableName, variable);
                variable.clazz.construct(compiler, classes, variables, variable, generics, methodName, args);
            }
            case INVOKE -> {
                Variable variable = variables.get(variableName);
                Usable sc = variable.clazz;
                sc.invoke(compiler, variables, variable, methodName, args);
            }
        }
    }
    ...
    static class Block implements language.core.Block {
        ...
        public void execute(Compiler.MethodCompiler compiler, Variable caller) {
            for (Statement stmt : block) {
                stmt.handle(compiler, classes, variables, caller);
            }
        }
    }
