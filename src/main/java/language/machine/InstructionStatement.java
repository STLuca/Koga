...
public class InstructionStatement implements Statement {
        ...
        switch (instructionType) {
            case ALLOCATE -> {
                ...
                for (int i = 1; i < this.arguments.size(); i+=2) {
                    ...
                    int val = resolveValue(inType, name, variable, arguments, context).value;
                    size *= val;
                }
            ...
            case CONTEXT -> {
                ...
                switch (type) {
                    ...
                    case VALUE -> {
                        ...
                        Resolved r = resolveValue(inType, resolveName, variable, arguments, context);
            ...
            case ADMIN -> {
                ...
                new InstructionStatement("c", "ADDR", "I", "LDA", methodAddr, "AL", methodSymbol).compile(compiler, variable, arguments, context);
                new InstructionStatement("l", "ADD", "RI", "LDA", frameDataAddr, "R", "altTask", "IL", "0d0").compile(compiler, variable, arguments, context);

                for (int i = 1; i < this.arguments.size(); i++) {
                    new InstructionStatement("m", "COPY", "PA", "LDA", frameDataAddr, "LDA", this.arguments.get(i), "IL", "0d4").compile(compiler, variable, arguments, context);
                    if (i == this.arguments.size() - 1) continue;
                    new InstructionStatement("l","ADD", "AI", "LDA", frameDataAddr, "LDA", frameDataAddr, "IL", "0d4").compile(compiler, variable, arguments, context);
                }

                new InstructionStatement("logician", "START_ADMIN", "LDA", methodAddr).compile(compiler, variable, arguments, context);
            }
            case L, LOGIC -> {
                ...
                Resolved rDest = resolveValue(destType, dest, variable, arguments, context);
                Resolved rSrc1 = resolveValue(src1Type, src1, variable, arguments, context);
                Resolved rSrc2 = resolveValue(src2Type, src2, variable, arguments, context);
            ...
            case J, JUMP -> {
                ...
                Resolved address = resolveValue(addrType, addr, variable, arguments, context);
                switch (inType) {
                    case A -> {
            ...
            case CB -> {
                ...
                Resolved resolvedSrc1 = resolveValue(src1Type, src1, variable, arguments, context);
                Resolved resolvedSrc2 = resolveValue(src2Type, src2, variable, arguments, context);
                Resolved resolvedAddr = resolveValue(InputType.LDA, addr, variable, arguments, context);
                ic.inputType(inType);
            ...
            case C, CLASS -> {
                ...
                Resolved resolvedDest = resolveValue(destType, dest, variable, arguments, context);
                Resolved resolvedTable = resolveValue(src1Type, src1, variable, arguments, context);
                ic.dest(resolvedDest.value(), resolvedDest.size());
                ic.src(resolvedTable.value(), resolvedTable.size());
                if (src2 != null) {
                    Resolved resolvedSymbol = resolveValue(src2Type, src2, variable, arguments, context);
            ...
            case M, MEMORY -> {
                ...
                Resolved resolvedDest = resolveValue(destType, dest, variable, arguments, context);
                Resolved resolvedSrc = resolveValue(srcType, src, variable, arguments, context);
                Resolved resolvedSize = resolveValue(sizeType, size, variable, arguments, context);
                ic.dest(resolvedDest.value(), resolvedDest.size());
            ...
            case I, INTERRUPT -> {
                ...
                Resolved resolvedSrc = resolveValue(InputType.LDA, src, variable, arguments, context);
                ic.src(resolvedSrc.value(), resolvedSrc.size());
            }
            case D, DEBUG -> {
                Compiler.InstructionCompiler ic = compiler.instruction();
                ...
                Resolved resolvedSrc = resolveValue(InputType.ADA, src, variable, arguments, context);
                Resolved resolvedSize = resolveValue(InputType.ADA, size, variable, arguments, context);
                ic.src(resolvedSrc.value(), resolvedSrc.size());
                ic.src(resolvedSize.value(), resolvedSize.size());
            }
            case LOGICIAN -> {
                ...
                switch (subType) {
                    case SET_OBJECT, SET_TABLE, START_ADMIN, SET_ALT_TASK, SET_ALT_OBJECT, SET_ALT_TABLE -> {
                        ...
                        Resolved resolvedSrc = resolveValue(srcType, values.get(2), variable, arguments, context);
                        ic.src(resolvedSrc.value, resolvedSrc.size);
                    }
                    case SET_METHOD_AND_TASK -> {
                        InputType src1Type = InputType.valueOf(values.get(1));
                        Resolved resolvedSrc1 = resolveValue(src1Type, values.get(2), variable, arguments, context);
                        InputType src2Type = InputType.valueOf(values.get(3));
                        Resolved resolvedSrc2 = resolveValue(src2Type, values.get(4), variable, arguments, context);
                        ic.src(resolvedSrc1.value, resolvedSrc1.size);
                        ic.src(resolvedSrc2.value, resolvedSrc2.size);
                    }
                    case STOP_ADMIN -> {}
    ...
    static Resolved resolveValue(InputType type, String toResolve, Variable variable, Map<String, Argument> arguments, Context context) {
        switch (type) {
            case R -> {
                switch (toResolve) {
                    case "task" -> {
                        return new Resolved(4, 0);
                    }
                    case "instruction" -> {
                        return new Resolved(4, 1);
                    }
                    case "altTask" -> {
                        return new Resolved(4, 2);
                    }
                    case "altInstruction" -> {
                        return new Resolved(4, 3);
                    }
                    case "object" -> {
                        return new Resolved(4, 4);
                    }
                    case "table" -> {
                        return new Resolved(4, 5);
                    }
                    case "altObject" -> {
                        return new Resolved(4, 6);
                    }
                    case "altTable" -> {
                        return new Resolved(4, 7);
                    }
                }
            }
            case IL -> {
